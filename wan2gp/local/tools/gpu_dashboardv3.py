...
def start_gddr6_reader():
    """Start gddr6 subprocess and return a thread-safe reader object."""
    gddr6_path = None
    # Check common locations
    for candidate in ["gddr6", "./gddr6", "/usr/local/bin/gddr6"]:
        try:
            if subprocess.run(["which", candidate], capture_output=True).returncode == 0:
                gddr6_path = candidate
                break
        except Exception:
            pass
    ...
    # Find first available GPU index for gddr6
    try:
    ...
    gddr6_idx = 0  # Use first GPU
    try:
        proc = subprocess.Popen(
            [gddr6_path, str(gddr6_idx)],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True,
            bufsize=1
        )
        return Gddr6Reader(proc)
    except Exception:
        return None

class Gddr6Reader:
    """Thread-safe reader for gddr6 output."""
    ...
    def _read_loop(self):
        """Read gddr6 output line by line."""
        while self._running:
            try:
                line = self.proc.stdout.readline().strip()
                if not line:
                    break
                # gddr6 outputs lines like: "VRAM Temp: 78°C" or "78°C"
                # Try to extract a number followed by °C
                import re
...
def main(stdscr):
    ...
    # Start gddr6 reader for VRAM temperature
    gddr6 = start_gddr6_reader()
    gddr6_available = gddr6 is not None
    gddr6_status_msg = "gddr6: ACTIVE" if gddr6_available else "gddr6: NOT FOUND (VRAM temp via clock drop)"
    ...
    try:
        while True:
            ...
            w(1, 2, f"  {time.strftime('%A %Y-%m-%d  %H:%M:%S')}    GPUs: {gpu_count}  {gddr6_status_msg}", MAGENTA)
            ...
            for i in range(gpu_count):
                ...
                # FALLBACK: use gddr6 VRAM temperature if NVML couldn't read it
                if t_mem == "N/A" and gddr6_available:
                    gddr6_temp = gddr6.get_temp()
                    if gddr6_temp is not None:
                        t_mem = gddr6_temp
