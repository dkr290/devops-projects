...
DANGER_REF = [
    ...
    (
        ...
        20,
        "%",
        "Sudden drop mid-workload = thermal/power throttle",
    ),
...
def color_vram_temp(t):
    """GDDR6X specific — RTX 3090 VRAM runs hot but degrades above 95°C"""
    return 2 if t < 80 else (3 if t < 92 else 1)
...
def get_sys_stats():
    ...
    try:
        ...
        if thermal_zones:
            zone = thermal_zones[0]
            temp_file = f"/sys/class/thermal/{zone}/temp"
            with open(temp_file) as f:
                cpu_temp = int(f.read().strip()) / 1000  # Convert m°C to °C
    except Exception:
        pass
    ...
    return cpu_pct, cpu_temp, mem_used, mem_total


# ── GDDR6 VRAM temp reader ───────────────────────────────────────────────────
def _gddr6_reader():
    """Background thread: runs gddr6 once and keeps reading its output."""
    try:
        ...
        for line in proc.stdout:
            if re.search(r"^Device:", line):
                gpu_idx += 1
            m = re.search(r"VRAM Temps:.*?\|\s*(\d+)[°\s]*C", line)
            if m and gpu_idx >= 0:
                _gddr6_cache["temps"][gpu_idx] = int(m.group(1))
                _gddr6_cache["ts"] = time.monotonic()
    except Exception:
        pass


def get_gddr6_temps():
    return _gddr6_cache["temps"]


threading.Thread(target=_gddr6_reader, daemon=True).start()
...
def main(stdscr):
    ...
    def ref_panel(row):
        w(row, 2, "─" * 80, DIM)
        row += 1
        w(row, 2, "  RTX 3090 DANGER REFERENCE  (h to hide)", CYAN | BOLD)
        row += 1
        w(row, 2, f"  {'METRIC':<22} {'WARN':>8}  {'DANGER':>9}   NOTE", MAGENTA)
        row += 1
        ...
        w(
            row,
            22,
            "9501 MHz = full speed    <9000 = light throttle    <7000 = heavy throttle",
            DIM,
        )
        row += 1
        w(
            row,
            2,
            "  HW Thermal ⚠  /  HW Slowdown ⚠  /  HW Power Brake ⚠  = hardware protecting card",
            RED,
        )
        row += 1
        w(
            row,
            2,
            "  VRAM clock sparkline: each char = 1 sample (1s). Dips = throttle events.",
            DIM,
        )
        return row + 1
    ...
    while True:
        ...
        w(
            1,
            2,
            f"  {time.strftime('%A %Y-%m-%d  %H:%M:%S')}    GPUs: {gpu_count}",
            MAGENTA,
        )
        ...
        # ── System (CPU + RAM) ───────────────────────────────────────────────
        cpu_pct, cpu_temp, mem_used, mem_total = get_sys_stats()
        ...
        if cpu_temp > 0:
            temp_color = C(color_temp(int(cpu_temp)))
            w(row, 25, f"{cpu_temp:.1f}°C", temp_color | BOLD)
            w(row, 32, f"{bar(int(cpu_temp), 100, 10)}", temp_color)
        else:
            w(row, 25, "N/A", DIM)

        # RAM
        w(row, 48, "RAM:", WHITE | BOLD)
        w(
            row,
            53,
            f"{mem_used:6} / {mem_total:6} MiB ({mem_pct}%)  {bar(mem_used, mem_total, 14)}",
            C(color_pct(mem_pct)),
        )
        row += 1
        ...
        for i in range(gpu_count):
            ...
            pci_str = (
                (pci.busId.decode() if isinstance(pci.busId, bytes) else pci.busId)
                if pci != "N/A"
                else "N/A"
            )
            ...
            w(
                row,
                11,
                f"GPU {t_gpu}°C {bar(t_gpu, 110, 12)}",
            ...
            if t_mem != "N/A":
                w(
                    row,
                    42,
                    f"  VRAM {t_mem}°C {bar(t_mem, 110, 12)}",
                ...
                w(
                    row,
                    42,
                    "  VRAM N/A → watch VRAM clock drop below and the temp from gddr6",
                    DIM,
                )
            row += 1
            ...
            if i in gddr6_temps:
                gt = gddr6_temps[i]
                w(row, 4, "VRAM Temp (gddr6):", WHITE | BOLD)
                w(row, 23, f"{gt}°C  {bar(gt, 120, 14)}", C(color_vram_temp(gt)) | BOLD)
                if gt >= 100:
                    w(row, 42, "  CRITICAL ⚠", RED | BOLD)
                elif gt >= 90:
                    w(row, 42, "  HOT ⚠", YELLOW | BOLD)
                elif gt >= 75:
                    w(row, 42, "  warm", YELLOW)
                else:
                    w(row, 42, "  OK ✓", GREEN)
            else:
                w(row, 4, "VRAM Temp (gddr6): unavailable (sudo?)", DIM)
            row += 1
            ...
            if t_slow != "N/A":
                w(
                    row,
                    4,
                    f"GPU temp Thresholds:  Slowdown {t_slow}°C   Shutdown {t_shut}°C",
                    DIM,
                )
                row += 1
            ...
            clk_mem = safe(
                pynvml.nvmlDeviceGetClockInfo, hnd, pynvml.NVML_CLOCK_MEM, default=0
            )
            clk_sm = safe(
                pynvml.nvmlDeviceGetClockInfo, hnd, pynvml.NVML_CLOCK_SM, default=0
            )
            clk_video = safe(
                pynvml.nvmlDeviceGetClockInfo, hnd, pynvml.NVML_CLOCK_VIDEO, default=0
            )
            clk_max_g = safe(
            ...
            v_base = vram_baseline[i] or (
                clk_max_m if clk_max_m != "N/A" else RTX3090_VRAM_BASELINE
            )
            g_base = gfx_baseline[i] or (clk_max_g if clk_max_g != "N/A" else 1)

            v_drop = (
                max(0, int((1 - clk_mem / v_base) * 100))
                if isinstance(clk_mem, int) and v_base > 0
                else 0
            )
            g_drop = (
                max(0, int((1 - clk_gfx / g_base) * 100))
                if isinstance(clk_gfx, int) and g_base > 0
                else 0
            )
            ...
            for f in range(fc):
                try:
                    ...
                    try:
                        fans.append(pynvml.nvmlDeviceGetFanSpeed(hnd))
                        break
                    except Exception:
                        pass
            fan_str = (
                "  ".join(f"Fan{j+1} {s}% {bar(s,100,8)}" for j, s in enumerate(fans))
                if fans
                else "N/A"
            )
            ...
            pw_lim = (
                safe(pynvml.nvmlDeviceGetPowerManagementLimit, hnd, default=1) / 1000
            )
            pw_def = (
                safe(pynvml.nvmlDeviceGetPowerManagementDefaultLimit, hnd, default=0)
                / 1000
            )
            ...
            if energy != "N/A":
                w(row, 4, f"Energy since boot: {energy/1000:.1f} kJ", DIM)
                row += 1
            ...
            extras = []
            if bw != "N/A":
                extras.append(f"bus: {bw}-bit")
            if ecc not in ("N/A", 0):
                extras.append(f"ECC uncorrected errors: {ecc} ⚠")
            if extras:
                w(row, 4, "  " + "   ".join(extras), DIM)
                row += 1
            ...
            try:
                ...
                tx = pynvml.nvmlDeviceGetPcieThroughput(
                    hnd, pynvml.NVML_PCIE_UTIL_TX_BYTES
                )
                rx = pynvml.nvmlDeviceGetPcieThroughput(
                    hnd, pynvml.NVML_PCIE_UTIL_RX_BYTES
                )
            ...
            try:
                ...
                w(
                    row,
                    4,
                    f"Perf State: P{ps}  (P0=max perf  P2=mid  P8=idle  P12=min)",
                    pc,
                )
                row += 1
            except Exception:
            ...
            # Process list
            w(row, 2, "├─ Active GPU Processes:", WHITE | BOLD)
            row += 1
            try:
            ...
            if not procs:
                w(row, 4, "  No active GPU processes", DIM)
                row += 1
            else:
                w(
                    row,
                    4,
                    f"  {'PID':<8} {'VRAM MiB':<12} {'TYPE':<10} Process",
                    MAGENTA,
                )
                row += 1
                for p in procs:
